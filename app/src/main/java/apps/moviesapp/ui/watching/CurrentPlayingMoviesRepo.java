...
public class CurrentPlayingMoviesRepo {
    private static CurrentPlayingMoviesRepo instance;
    ...
    private CurrentPlayingMoviesRepo() {
    }

    public static CurrentPlayingMoviesRepo getInstance() {
        if (instance == null) {
            synchronized (mutex) {
                if (instance == null) {
                    instance = new CurrentPlayingMoviesRepo();
                }
            }
        }
    ...
    public MutableLiveData<MoviesResponse> getCurrentPlayingMovies(int pageNumber) {
        handelError = new MutableLiveData<>();
        MutableLiveData<MoviesResponse> moviesMutableLiveData = new MutableLiveData<>();

        Call<MoviesResponse> call = apiService.getCurrentPlayingMovies(Constant.REQUESTS_LANGUAGE, pageNumber);
